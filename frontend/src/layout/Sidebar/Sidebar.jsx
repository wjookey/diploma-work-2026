...
import {
    LayoutDashboard, Users, GraduationCap, Palette, CalendarDays,
    CreditCard, FileText, BookOpen, Clipboard, AlarmClock
} from 'lucide-react';
...
const Sidebar = ({ isOpen, onToggle, onExitButtonClick, name, email }) => {
    ...
    const sidebarLinks = adminLinks.map((link) => (
        <SidebarLink key={link.to} keyValue={link.to} toValue={link.to} endValue={link.to === '/'} icon={link.icon} pageName={link.label} />
    ));
    
    return (
        <div className={styles.wrapper}>
            <div className={`${styles.darkBackground} ${isOpen ? styles.open : styles.close}`} onClick={onToggle} />
